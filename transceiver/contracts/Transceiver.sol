...
contract Transceiver {
    ...
    event NoticeStatusUpdate(NoticeStatusEnum status, uint64 noticeID, uint64 gsmNumber, uint256 clientTsMs);

    // events created for just test purposes
    ...
    /// @param clientTsMs test parameter to record response time
    function updateNoticeStatus(
        ...
        if (shouldEmit) {
            ...
            emit NoticeStatusUpdate(status, noticeID, gsmNumber, clientTsMs);
        }
    }
}
